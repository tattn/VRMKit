...
/// Giving a merged prop its swing. The two VRM versions describe one at different grains,
/// so each is written through its own entry point.
@Suite
struct VRMSpringBoneWritingTests {
    // MARK: - Round trip

    /// VRM 1.0 states the parameters per joint. The values are ones a `Float` holds
    /// exactly, since they are read back as the `Double` the JSON carries.
    @Test
    func testAVRM1SpringIsWrittenWithItsJoints() throws {
        ...
        let written = try #require(try vrm1(of: document).springBone?.springs?[index.rawValue])
    ...
    /// A chain may stiffen towards its root, which is why the parameters are stated per
    /// joint rather than per spring.
    @Test
    func testAVRM1SpringCanGiveEachJointItsOwnSettings() throws {
        ...
        let spring = try #require(try vrm1(of: document).springBone?.springs?[index.rawValue])
        #expect(spring.joints.map(\.stiffness) == [1, 0.25, nil])
    }

    /// Nothing is written for the last joint, which is only a tail, nor for a joint at
    /// the default: reading fills both in.
    @Test
    func testAVRM1SpringWritesNoParameterItDoesNotNeedTo() throws {
        ...
        let joints = try #require(springBoneJSON(of: document)[safe: index.rawValue]).objects("joints")
        #expect(joints.map { Set($0.keys) } == [["node", "stiffness"], ["node", "stiffness"], ["node"]])
    }

    /// VRM 0.x names the nodes a swing starts at and swings what hangs below them, so a
    /// group may name several roots and swings their side branches.
    @Test
    func testAVRM0BoneGroupIsWrittenWithItsRootBones() throws {
        ...
        let written = try #require(try vrm0(of: document).secondaryAnimation.boneGroups[safe: index.rawValue])
        #expect(written.bones == [left[0].rawValue, right[0].rawValue])
        #expect(written.comment == "charm")
        #expect(written.colliderGroups == [1])
        #expect(written.stiffness == 0.75)
    ...
    func testAddingASpringLeavesTheModelsOwnSpringsAlone(asset: VRMSampleAsset) throws {
        ...
        try document.addSpringBone(over: ornament, of: asset)
    ...
    /// What holds a spring is a declared `VRMC_springBone`, or a 0.x model's `secondaryAnimation`.
    @Test(arguments: [VRMSampleAsset.aliciaSolid, .seedSan])
    ...
    func testAnAddedSpringSurvivesPruning() throws {
        ...
        let joints = try #require(springs[safe: index.rawValue]).joints.map(\.node)
    ...
    func testParametersOutsideWhatTheFormatsAcceptAreRefused(asset: VRMSampleAsset) throws {
        var document = try GLTFEditableDocument(data: asset.data)
        let ornament = try document.addOrnamentChain(under: asset.headNode)
        let nodeCount = try document.typed().nodes.count
        let before = try document.serialize()

        let refused: [(VRM1Spring) -> VRM1Spring] = [
            { var spring = $0; spring.center = GLTFNodeIndex(nodeCount); return spring },
    ...
    /// A VRM 1.0 spring names a line the hierarchy runs down, and needs a node past the
    /// last joint to swing it towards.
    @Test
    func testAVRM1SpringOverNodesThatAreNotALineIsRefused() throws {
        ...
        let refused: [[GLTFNodeIndex]] = [
            [],
            [ornament[0], GLTFNodeIndex(nodeCount)],
            [ornament[0], -1],
    ...
    /// One node belongs to one spring. A node the spring skips over is part of its chain
    /// all the same, so it is spoken for too.
    @Test
    ...
    /// The center is the first joint or one of its ancestors, and neither it nor anything
    /// it hangs off is swung by another spring.
    @Test
    ...
    /// The two forms are not interchangeable, so each is refused by the version that does
    /// not keep it.
    @Test
    ...
    /// `VRMC_springBone` is versioned on its own, so a document declaring another version
    /// is not given springs of this shape, `1.0-beta` included.
    ...
    /// What a spring may swing is decided against the springs already there, so a document
    /// whose own springs cannot be read is refused.
    ...
    /// Only what the edit reads has to make sense: a collider the new spring never names
    ...
    func testASpringIsAddedBesideAColliderThisEditNeverReads() throws {
        ...
        #expect(springs[safe: index.rawValue]?.objects("joints").compactMap { $0.index("node") } == ornament.map(\.rawValue))
    }
    ...
    /// The hierarchy an edit reads is the one a loader reads, so a document a loader
    /// refuses is not one a spring is written into.
    @Test
...
private extension GLTFEditableDocument {
    /// The same line of nodes written as whichever form `asset` keeps, for the assertions
    /// that hold whatever version a model is.
    ...
    mutating func addSpringBone(_ spring: VRM1Spring, of asset: VRMSampleAsset) throws {
        guard try asset.isVRM0 else {
            try addVRM1SpringBone(spring)
            return
        }
        let joint = spring.joints[0]
        try addVRM0SpringBone(VRM0SpringBoneGroup(
