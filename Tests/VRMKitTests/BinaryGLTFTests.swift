...
struct BinaryGLTFTests {
    ...
    /// A buffer view is a slice of its buffer rather than a copy of it, so an
    /// accessor's offset counts from where the view starts, not from where the
    /// buffer does.
    @Test
    func testSubdataOfASliceCountsFromTheSliceStart() throws {
        ...
        // The view bounds the accessor: what the buffer holds past it is not
        // part of it, and the bytes before it are not addressable.
    ...
    /// UniVRM 0.x appends a model's thumbnail past the `byteLength` it declares
    ...
    func testBufferViewDataRejectsAnIndexBeyondTheFile() throws {
        ...
        #expect(throws: (any Error).self) { try document.bufferViewData(at: document.gltf.bufferViews?.count ?? 0) }
    }
    ...
    func testLoadsAFileWithBytesPastTheHeaderLength() throws {
        ...
        #expect(binaryGltf.jsonData.asset.version == "2.0")
