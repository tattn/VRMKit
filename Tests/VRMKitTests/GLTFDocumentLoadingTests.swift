...
struct GLTFDocumentLoadingTests {
    ...
    /// A base URL without a trailing slash names a file, so resolving a
    /// relative `uri` against it would look beside the directory rather than
    /// inside it.
    @Test
    ...
    func testNodeRotationDefaultsToIdentity() throws {
        ...
        """
        let nodes = try #require(GLTFDocument(data: Data(json.utf8)).gltf.nodes)
