...
struct GLTFMToonTests {
    /// VRM 0.x keeps MToon in the Unity material property, so a converted
    /// material replaces the entry that would have said "render the glTF
    /// material as it is".
    @Test
    func testAppendingAsMToonWritesAVRM0MaterialProperty() throws {
        ...
        // The fixture's material is white, so the shade color is the scale
        // itself, written back in the sRGB space VRM 0.x keeps colors in.
    ...
    /// Everywhere else MToon is the glTF extension, next to the unlit fallback
    /// it names for clients that do not implement it.
    @Test
    func testAppendingAsMToonWritesTheMToonExtension() throws {
        ...
        let used = try #require(reloaded.gltf.extensionsUsed)
    ...
    /// An outline is the one part of the style with a unit to get wrong: MToon
    /// 1.0 measures world-space width in meters and VRM 0.x in centimeters.
    @Test
    ...
    /// VRM 0.x applies one UV transform to every texture of a material and
    /// samples UV set 0 only, so a rotation or a second UV set cannot be
    /// written without the model coming out shaded differently.
    ...
    /// A transform every texture shares is what VRM 0.x does carry, as the
    /// material's `_MainTex` scale and offset.
    @Test
    ...
    /// Keeping the materials is still the default, and still writes the entry
    ...
    /// `materialProperties` runs parallel to `materials`, and glTF lets two
    /// materials share a name, so only the index tells them apart.
    @Test
    func testMaterialSettingsAreResolvedByIndexNotByName() throws {
        ...
        let materials = try #require(vrm0.document.gltf.materials)
        #expect(materials.count > 1)

        // The materials differ in shader despite now sharing a name, so a
        // name-keyed lookup could only answer with one of them.
