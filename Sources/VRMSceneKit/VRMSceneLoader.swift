...
open class VRMSceneLoader {
    ...
    public init(vrm: VRM, rootDirectory: URL? = nil) {
        ...
        self.scnData = SceneData(vrm: gltf)
    }
    ...
    func bufferView(withBufferViewIndex index: Int) throws -> (bufferView: Data, stride: Int?) {
        ...
        if let cache = try scnData.load(\.bufferViews, index: index) { return (cache, gltfBufferView.byteStride) }
    ...
    private func buffer(withBufferIndex index: Int) throws -> Data {
        ...
        scnData.buffers[index] = buffer
        return buffer
    }

    func material(withMaterialIndex index: Int) throws -> SCNMaterial {
        if let cache = try scnData.load(\.materials, index: index) { return cache }
        ...
        scnData.materials[index] = material
        return material
    }

    func texture(withTextureIndex index: Int) throws -> SCNMaterialProperty {
        if let cache = try scnData.load(\.textures, index: index) { return cache }
        ...
        scnData.textures[index] = texture
        return texture
    }

    func image(withImageIndex index: Int) throws -> UIImage {
        if let cache = try scnData.load(\.images, index: index) { return cache }
